 ...
 * Version 0.11 - Added Exit menu option, updated defaults
 * 
 ...
 * 
 * Changes from v0.10:
 *   - Added "Exit" as first menu option for easy navigation
 *   - Changed default setpoint to 375 ml
 *   - Changed default K-factors to 0.24 pulses/ml
 */
...
float tempKFactor = 5; // Temporary K-factor value while adjusting

// K-factors: pulses per ml (calibrate for each flowmeter) - DEFAULT 5.0
float kFactor[4] = {5.0,5.0,5.0,5.0};

// Cycle counters - track total completed cycles per channel
...
void setup() {
  ...
  Serial.printf("Setpoint: %.1f ml\n", setpointMl);
  Serial.printf("K-Factors: %.4f, %.4f, %.4f, %.4f\n", 
                kFactor[0], kFactor[1], kFactor[2], kFactor[3]);
  
  // Setup counter pins with internal pull-up
  ...
  // Debug: confirm all channels are IDLE
  ...
  if (Serial.available()) {
    ...
    if (cmd.startsWith("SP=")) {
      ...
      if (newSp > 0 && newSp < 100000) {
        setpointMl = newSp;
        saveSetpoint();
        Serial.printf("Setpoint changed to %.1f ml\n", setpointMl);
        forceDisplayUpdate = true;
        drawStaticGUI();
      }
      ...
      if (newK > 0) {
        kFactor[0] = newK;
        saveKFactor(0);
        Serial.printf("K-Factor 1 changed to %.4f\n", kFactor[0]);
      }
      ...
      if (newK > 0) {
        kFactor[1] = newK;
        saveKFactor(1);
        Serial.printf("K-Factor 2 changed to %.4f\n", kFactor[1]);
      }
      ...
      if (newK > 0) {
        kFactor[2] = newK;
        saveKFactor(2);
        Serial.printf("K-Factor 3 changed to %.4f\n", kFactor[2]);
      }
      ...
      if (newK > 0) {
        kFactor[3] = newK;
        saveKFactor(3);
        Serial.printf("K-Factor 4 changed to %.4f\n", kFactor[3]);
      }
    } else if (cmd == "STATUS") {
      Serial.printf("Setpoint: %.1f ml\n", setpointMl);
      Serial.printf("K-Factors: K1=%.4f K2=%.4f K3=%.4f K4=%.4f\n", 
                    kFactor[0], kFactor[1], kFactor[2], kFactor[3]);
      for (int i = 0; i < 4; i++) {
        const char* stateStr;
  ...
  if (displayMode == MAIN_SCREEN) {
        ...
        switch (channelState[ch]) {
          ...
          case RUNNING:
            if (ml >= setpointMl) {
              ...
              saveCycleCounts();  // Save to non-volatile memory
              forceDisplayUpdate = true;
              Serial.printf("CH%d: Complete (%.1f ml) - Total cycles: %lu\n", ch + 1, ml, cycleCount[ch]);
              ...
              forceDisplayUpdate = true;
              Serial.printf("CH%d: FAILED at %.1f ml (counter frozen, input ON to restart)\n", ch + 1, ml);
            }
            break;
    ...
    if (millis() - lastDebug >= DEBUG_INTERVAL) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        Serial.printf("CH%d:%s %.1fml L:%d ", i + 1, stateStr, countsToMl(i, getCounter(i)), remoteLatched[i]);
      }
      Serial.println();
    }
  }
...
void loadSettings() {
  ...
  // New defaults: 375ml setpoint, 0.24 k-factor
  setpointMl = prefs.getFloat("setpoint", 375.0);
  kFactor[0] = prefs.getFloat("kfactor1", 0.24);
  kFactor[1] = prefs.getFloat("kfactor2", 0.24);
  kFactor[2] = prefs.getFloat("kfactor3", 0.24);
  kFactor[3] = prefs.getFloat("kfactor4", 0.24);
  
  prefs.end();
  
  Serial.println("Settings loaded from preferences");
  Serial.println("Cycle counts reset to zero on power up");
}
...
void saveCycleCounts() {
  // Cycle counts are NOT saved - they reset to zero on every power up
...
void resetAllCycleCounts() {
  ...
  saveCycleCounts();
  Serial.println("All cycle counts reset to zero");
}
...
void drawStaticGUI() {
  ...
  // Header row (with 5px left margin) - added Units column
  M5StamPLC.Display.setCursor(7, 18);
  M5StamPLC.Display.printf("CH IN OUT   ml  STATE UNITS");
...
void drawChannelStatus(int ch) {
  ...
  if (inputIndicator != lastInputDisplay[ch] || forceDisplayUpdate) {
    if (inputIndicator) {
  ...
  if (channelState[ch] != lastState[ch] || forceDisplayUpdate) {
    if (outputOn) {
  ...
  // ml value (adjusted position)
  // When FAILED, counter is frozen - only update on state change or significant value change
  if (channelState[ch] == FAILED) {
    // Only update display on state change to FAILED or force update
    if (channelState[ch] != lastState[ch] || forceDisplayUpdate) {
      M5StamPLC.Display.fillRect(62, yPos, 38, 16, TFT_BLACK);
      M5StamPLC.Display.setTextColor(TFT_CYAN);
      M5StamPLC.Display.setCursor(62, yPos + 4);
      M5StamPLC.Display.printf("%5.1f", currentMl);
      lastMl[ch] = currentMl;
    }
  } else {
    // Normal update for non-FAILED states
    if (abs(currentMl - lastMl[ch]) >= 0.5 || forceDisplayUpdate) {
      M5StamPLC.Display.fillRect(62, yPos, 38, 16, TFT_BLACK);
      M5StamPLC.Display.setTextColor(TFT_CYAN);
      M5StamPLC.Display.setCursor(62, yPos + 4);
      M5StamPLC.Display.printf("%5.1f", currentMl);
  ...
  // Cycle count (new column)
  if (cycleCount[ch] != lastCycleCount[ch] || forceDisplayUpdate) {
    M5StamPLC.Display.fillRect(138, yPos, 20, 16, TFT_BLACK);
...
void handleButtons() {
  ...
  if (displayMode == MAIN_SCREEN) {
    // Button C enters settings menu - only if all channels are IDLE
    if (M5StamPLC.BtnC.wasPressed()) {
      // Check if all channels are IDLE
      bool allIdle = true;
      for (int i = 0; i < 4; i++) {
        if (channelState[i] != IDLE) {
          allIdle = false;
          break;
        }
      }
      ...
      if (allIdle) {
        ...
        Serial.println("Settings blocked - channels not IDLE");
        M5StamPLC.Display.fillRect(0, 110, M5StamPLC.Display.width(), 10, TFT_RED);
        M5StamPLC.Display.setTextColor(TFT_WHITE);
        M5StamPLC.Display.setCursor(10, 112);
        M5StamPLC.Display.printf("Wait for IDLE");
        delay(1000);
        forceDisplayUpdate = true;
      }
    }
    ...
    if (M5StamPLC.BtnC.wasPressed()) {
      ...
      drawSettingsMenu();
      lastButtonPress = millis();
      Serial.printf("K-factor CH%d saved: %.4f\n", kFactorChannel + 1, tempKFactor);
    }
...
void drawButtonLegends() {
  ...
  M5StamPLC.Display.fillRect(0, 120, M5StamPLC.Display.width(), 8, TFT_BLACK);
  ...
  // Show "config" above BtnC (right side) if all channels are IDLE
  if (allIdle) {
    M5StamPLC.Display.setTextSize(1);
    M5StamPLC.Display.setTextColor(TFT_CYAN);
    M5StamPLC.Display.setCursor(M5StamPLC.Display.width() - 36, 120);
...
void sendStatusJSON() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    jsonStr += ",\"ml\":";
    jsonStr += String(countsToMl(i, getCounter(i)), 2);
    jsonStr += ",\"kfactor\":";
    jsonStr += String(kFactor[i], 4);
    jsonStr += ",\"cycles\":";
  ...
  if (Serial2.available()) {
    ...
    if (cmd.startsWith("SP=")) {
      ...
      if (newSp >= 5.0 && newSp <= 9999.0) {
        setpointMl = newSp;
        saveSetpoint();
        response = "{\"response\":\"OK\",\"setpoint\":" + String(setpointMl, 1) + "}";
        if (displayMode == MAIN_SCREEN) {
          forceDisplayUpdate = true;
          drawStaticGUI();
        }
      ...
      if (ch >= 0 && ch < 4 && newK >= 0.01 && newK <= 100.0) {
        kFactor[ch] = newK;
        saveKFactor(ch);
        response = "{\"response\":\"OK\",\"channel\":" + String(ch + 1) + ",\"kfactor\":" + String(kFactor[ch], 4) + "}";
      } else {
      ...
      response = "{\"commands\":[\"SP=value\",\"K1=value\",\"K2=value\",\"K3=value\",\"K4=value\",\"START1\",\"START2\",\"START3\",\"START4\",\"STOP1\",\"STOP2\",\"STOP3\",\"STOP4\",\"RESETCYCLES\",\"STATUS\",\"HELP\"]}";
    } 
    else {
