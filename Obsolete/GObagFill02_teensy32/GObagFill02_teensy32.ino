...
const int ledPin = 13;
const int SWpin[] = {2,3,4,8};
const int PMPpin[] = {5,6,9,10};        // support by PWM if needed
const int FMpin[] = {23,22,21,20};      // these are pins supported by freqmeasuremulti
const float count2ml = 1.0;
...
void loop() {
  ...
  if ((millis() - onesecMillis) > 1000) {         // reporting only
    ...
    Serial.print("analog 12 is: ");
    Serial.println(val);
   
    /*
    if (count[0] > 0) {
      Serial.print(freq0.countToFrequency(sum[0] / count[0]));
      Serial.print(" ' ");Serial.println(cum_count[0]);  
  } else {
    Serial.print(" FM0 (no pulses)");
  }
