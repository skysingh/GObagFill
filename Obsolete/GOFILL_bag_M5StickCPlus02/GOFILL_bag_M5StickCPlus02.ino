...
void setup() {
    ...
    while (!relay.begin(&Wire, 32, 33)){      // fixed 0x26
    ...
    while (not_empty) {
      ...
      for (uint8_t i = 0; i < 4; i++) {                // check all bag holder must be empty on startup
         SWState[i] = extio.getDigitalInput(i);        // 1 = open => empty
...
void checkSwitch(int num) {
  int reading = extio.getDigitalInput(num);
...
void showSwitches() {
   for (int i=0;i<4;i++) {
      if (SWState[i] == 0) {
        M5.Lcd.fillCircle(26, 37 + i*35,8,GREEN);   // 0 => closed
      }  
      else {
